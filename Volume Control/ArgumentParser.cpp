...
void ArgumentParser::parse() {
    ...
    for (size_t i = 0; i < argv_.size(); ++i) {
        ...
        // Check if it's a switch (starts with - or --)
        if (arg.size() > 1 && (arg[0] == '-')) {
            ...
            for (auto& [name, switch_info] : switches_) {
                ...
                if (arg == full_switch_name) {
                    ...
                    if (switch_info.type == SwitchType::PARAMETER) {
                        ...
                        if (i + 1 >= argv_.size()) {
                            handle_missing_parameter(switch_info, i);
                            break;
                        }
                        
                        const std::string_view potential_param = argv_[i + 1];
                        
                        // Check if next argument is another switch
                        if (potential_param.size() > 1 && potential_param[0] == '-') {
                            handle_missing_parameter(switch_info, i);
                            break;
                        }
                        
                        // Set the parameter value and skip it in next iteration
                        switch_info.value = std::string(potential_param);
                        ++i;
                    }
                    
                    // Also set the paired switch if it exists
...
std::string ArgumentParser::format_switch_list() const {
    ...
    for (const auto& [name, switch_info] : switches_) {
        ...
        if (!switch_info.pair_name.empty()) {
            ...
            result += ", " + paired_switch.prefix + paired_switch.name;
        }
