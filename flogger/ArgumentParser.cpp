...
void ArgumentParser::printHelp(const std::string& header, bool hasNonSwitchArguments, bool wideString) const {
	...
	if (!wideString) {
		...
		std::wcout << wmessage << L'\n';
	}
}
void ArgumentParser::printHelp(const std::wstring& header, bool hasNonSwitchArguments) const {
	const std::wstring executableNameW(this->executableName.begin(), this->executableName.end());
	const std::wstring formattedSwitchLineW = std::wstring(this->formatSwitchLine(hasNonSwitchArguments).begin(), this->formatSwitchLine(hasNonSwitchArguments).end());
	const std::wstring formattedSwitchListW = std::wstring(this->formatSwitchList().begin(), this->formatSwitchList().end());
	const std::wstring wmessage =
		L'\n' +
		header +
		L"\n\nUsage:\n  " +
		executableNameW + L' ' + formattedSwitchLineW +
		L"\n\nSwitches:\n" +
		formattedSwitchListW;
		std::wcout << wmessage << L'\n';
}
