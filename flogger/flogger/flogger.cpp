...
static void buildArgumentParser(ArgumentParser &parser) {
	...
	parser.addSwitchPair("n", "name", "Log file name. Defaults to 'logger.<EXT>'.",
		ArgumentParser::HASPARAM, ArgumentParser::THROWIFNOTSET);
	parser.addSwitchPair("e", "extension", "Log file extension. Defaults to 'log'.",
		ArgumentParser::HASPARAM, ArgumentParser::THROWIFNOTSET);
	parser.addSwitchPair("o", "overwrite", "Overwrite existing log file. Defaults to append.",
		ArgumentParser::NOPARAM, ArgumentParser::NOTHROWIFNOTSET);
	parser.addSwitchPair("t", "timestamp", "Prefix each log entry with a timestamp.",
		ArgumentParser::NOPARAM, ArgumentParser::NOTHROWIFNOTSET);
}
...
int main(int argc, char* argv[]) {
	...
	fileutils::FileInfo logFile(
		std::filesystem::path(
				std::filesystem::path(
					argument_parser.isSwitchSet("p") ?
				...
				std::filesystem::path(
					...
					((argument_parser.getSwitchValue("n") + '.') +
						(argument_parser.isSwitchSet("e") ?
							argument_parser.getSwitchValue("e") : "log"
							)
						) :
					("logger." +
						(argument_parser.isSwitchSet("e") ?
